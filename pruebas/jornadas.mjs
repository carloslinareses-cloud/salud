...
try {
  ...
  await pag.select('#joEvParroquia', 'CHARALLAVE')
  await espera(300)
  const comunas = await pag.$$eval('#joEvComuna option', o => o.length)
  prueba('la comuna se elige de una lista, no se escribe', comunas > 3, String(comunas))
  const comuna = await pag.$eval('#joEvComuna option:nth-child(2)', o => o.value)
  await pag.select('#joEvComuna', comuna)
  await espera(300)
  const comunidad = await pag.$eval('#joEvComunidad option:nth-child(2)', o => o.value)
  await pag.select('#joEvComunidad', comunidad)
  prueba('la comunidad depende de la comuna elegida', (await pag.$$eval('#joEvComunidad option', o => o.length)) > 1)
